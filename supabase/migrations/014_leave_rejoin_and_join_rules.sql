-- Rolli: leave / rejoin and hardened join rules
...
BEGIN
  ...
  IF v_hangout.status = 'waiting' AND v_participant.is_film_keeper IS TRUE THEN
    RAISE EXCEPTION 'The Film Keeper cannot leave while the hangout is waiting. Start or end the hangout first.';
  END IF;
...
-- ---------------------------------------------------------------------------
-- rejoin_hangout — reactivate a participant who previously left
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- join_hangout — reactivate inactive nickname; block duplicate active names
-- ---------------------------------------------------------------------------
...
GRANT EXECUTE ON FUNCTION public.leave_hangout(UUID, UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rejoin_hangout(TEXT, UUID) TO anon, authenticated;

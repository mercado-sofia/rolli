-- Allow reveal preload during developing; signal when Film Keeper taps Start reveal.

ALTER TABLE public.hangouts
  ADD COLUMN IF NOT EXISTS reveal_pending_at TIMESTAMPTZ;
...
AS $$
  SELECT jsonb_build_object(
    ...
    'reveal_pending_at', p_hangout.reveal_pending_at
  );
$$;

CREATE OR REPLACE FUNCTION public.signal_reveal_pending(
  p_hangout_id UUID,
...
BEGIN
  ...
  IF v_hangout.status <> 'developing' THEN
    RAISE EXCEPTION 'Reveal can only be signaled while memories are developing';
  END IF;

  UPDATE public.hangouts
  SET reveal_pending_at = now()
...
AS $$
  SELECT EXISTS (
    ...
    WHERE ph.storage_path = p_path
      AND h.status IN ('developing', 'revealing', 'guessing', 'completed')
  );
$$;

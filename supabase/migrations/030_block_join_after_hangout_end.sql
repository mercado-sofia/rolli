-- Rolli: block new guests after the host ends capture (developing+).
-- Late join remains allowed only while status is waiting or active.
-- Returning participants use rejoin_hangout with their session token.
